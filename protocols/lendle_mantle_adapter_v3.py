...
ATOKEN_V3_ABI = load_abi('atoken_abi.json')

class LendleMantleAdapterV3(BaseProtocolAdapter):
    SOURCE = "Lendle Mantle V3"
    PROTOCOL_NAME = "Lendle"
    NETWORK_NAME = "Mantle"
    LEND_TOKEN_ADDRESS = "0x25356aeca4210ef7553140edb9b8026089e49396"
    

    RPC_URLS = ["https://mantle-mainnet.public.blastapi.io"]
    ...
    def get_web3_provider(cls):
        ...
        cached_provider_data = cache_manager.get_cached_data('lendle_v3', 'mantle', 'web3_provider')
        if cached_provider_data:
            try:
                ...
                cache_manager.invalidate_cache('lendle_v3', 'mantle', 'web3_provider')
        ...
        for rpc_url in cls.RPC_URLS:
            try:
                ...
                if provider.is_connected():
                    cache_manager.set_cached_data('lendle_v3', 'mantle', 'web3_provider', {
                        'rpc_url': rpc_url
                    }, ttl=900)
                    return provider
            except Exception:
    ...
    def get_underlying_asset_address(cls, atoken_address: str) -> str:
        ...
        cached_address = cache_manager.get_cached_data('lendle_v3', 'mantle', cache_key)
        if cached_address:
        ...
        try:
            ...
            cache_manager.set_cached_data('lendle_v3', 'mantle', cache_key, {
    ...
    def get_lend_price(cls) -> float:
        cached_price = cache_manager.get_cached_data('lendle_v3', 'mantle', 'lend_price')
        if cached_price:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('pairs') and len(data['pairs']) > 0:
                    ...
                    cache_manager.set_cached_data('lendle_v3', 'mantle', 'lend_price', {
                        'price': price
                    }, ttl=3600)
                    return price
        except Exception:
    ...
    def fetch_reserve_data(cls, fund_data, wallet_address):
        ...
        try:
            # Get aToken address
            atoken_address = Web3.to_checksum_address(fund_data['address'])
            ...
            cached_static = cache_manager.get_cached_data('lendle_v3', 'mantle', static_cache_key)
            
            dynamic_cache_key = f"dynamic_{underlying_address}"
            cached_dynamic = cache_manager.get_cached_data('lendle_v3', 'mantle', dynamic_cache_key)
            
            contracts_cache_key = f"contracts_{underlying_address}"
            cached_contracts = cache_manager.get_cached_data('lendle_v3', 'mantle', contracts_cache_key)
            ...
            if cached_contracts:
                a_token_address_from_pool = cached_contracts['a_token_address']
                ...
                reserve_data = lending_pool.functions.getReserveData(underlying_address).call()
                a_token_address_from_pool = reserve_data[7]
                variable_debt_token_address = reserve_data[9]
                interest_rate_strategy_address = reserve_data[10]
                
                cache_manager.set_cached_data('lendle_v3', 'mantle', contracts_cache_key, {
                    'a_token_address': a_token_address_from_pool,
            ...
            if cached_dynamic:
                ...
                a_token = web3.eth.contract(address=a_token_address_from_pool, abi=ATOKEN_ABI)
                ...
                cache_manager.set_cached_data('lendle_v3', 'mantle', dynamic_cache_key, {
            ...
            if cached_static:
                ...
                try:
                    static_batch_calls = [
                        ...
                        data_provider.functions.getReserveConfigurationData(underlying_address)
                    ]
                    ...
                    reserve_config = data_provider.functions.getReserveConfigurationData(underlying_address).call()
                    reserve_factor = reserve_config[4]
                ...
                cache_manager.set_cached_data('lendle_v3', 'mantle', static_cache_key, {
                    'name': reserve_name,
            ...
            logging.error(f"Lendle V3 error after {total_time:.3f}s: {str(e)}")
            return None